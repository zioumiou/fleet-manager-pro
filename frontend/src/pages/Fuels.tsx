  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        total_cost: parseFloat(calculateTotal(parseFloat(String(formData.liters)), parseFloat(String(formData.price_per_liter)))),
        full_tank: parseInt(String(formData.full_tank))
      };
  ...
  return (
    <div className="p-2">
      <div className="flex justify-between items-center mb-8">
        ...
        <button 
          onClick={() => { setShowForm(true); setEditingId(null); setFormData({ vehicle_id: 0, fuel_date: new Date().toISOString().split('T')[0], mileage: 0, liters: 0, price_per_liter: 0, total_cost: 0, station: '', full_tank: 1 }); }} 
        ...
        <div className="bg-white p-6 rounded-lg shadow-md mb-6 border-l-4 border-blue-500">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="date" required value={formData.fuel_date} onChange={(e) => setFormData({...formData, fuel_date: e.target.value})} className="w-full border rounded-lg px-3 py-2" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" step="0.01" required value={formData.liters} onChange={(e) => setFormData({...formData, liters: parseFloat(e.target.value) || 0})} className="w-full border rounded-lg px-3 py-2" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Prix/L (DA) *</label>
              <input type="number" step="0.001" required value={formData.price_per_liter} onChange={(e) => setFormData({...formData, price_per_liter: parseFloat(e.target.value) || 0})} className="w-full border rounded-lg px-3 py-2" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Coût Total (DA)</label>
              <input type="number" step="0.01" value={calculateTotal(formData.liters, formData.price_per_liter)} readOnly className="w-full border rounded-lg px-3 py-2 bg-gray-100" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Station</label>
              <input type="text" value={formData.station} onChange={(e) => setFormData({...formData, station: e.target.value})} className="w-full border rounded-lg px-3 py-2" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Plein complet</label>
              <select value={formData.full_tank} onChange={(e) => setFormData({...formData, full_tank: parseInt(e.target.value)})} className="w-full border rounded-lg px-3 py-2 bg-white">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {fuels.length === 0 ? (
              <tr><td colSpan={8} className="px-6 py-8 text-center text-gray-500">Aucun plein enregistré</td></tr>
            ) : (
              fuels.map((f) => (
                <tr key={f.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">{f.price_per_liter.toFixed(3)} DA</td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      <button onClick={() => handleEdit(f)} className="text-blue-600 hover:text-blue-800 p-1 hover:bg-blue-50 rounded"><Edit size={18} /></button>
                      <button onClick={async () => { if(confirm('Supprimer ?')) { await deleteFuel(f.id); loadData(); } }} className="text-red-600 hover:text-red-800 p-1 hover:bg-red-50 rounded"><Trash2 size={18} /></button>
                    </div>
                  </td>
                </tr>
              ))
