...
const getStatusBadge = (status: string, days: number | null | undefined, km: number | null | undefined) => {
    ...
    if (status === 'red') text = "Dépassé !";
      ...
      const parts = [];
      if (days !== null && days <= 30) parts.push(`${days}j`);
  ...
  return (
    <div className="p-2">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Rappels & Entretiens</h1>
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-lg shadow-md mb-6 border-l-4 border-blue-500">
          <h2 className="text-xl font-semibold mb-4">{editingId ? 'Modifier le rappel' : 'Nouveau Rappel'}</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Type de rappel *</label>
              <select 
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Kilométrage d'échéance (Optionnel)</label>
              <input 
                type="number" 
            ...
            <div className="md:col-span-2">
              ...
              <input 
                ...
                className="w-full border rounded-lg px-3 py-2" 
                placeholder="Ex: Huile 5W30, Garage XYZ..." 
              />
            </div>
        ...
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Échéance Date</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Échéance Km</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {reminders.length === 0 ? (
              <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">Aucun rappel configuré</td></tr>
            ) : (
              reminders.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50 transition">
                  <td className="px-6 py-4 font-semibold">{r.vehicle_plate}</td>
